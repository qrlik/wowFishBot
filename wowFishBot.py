    ...
    with mss.mss() as screenshotManager:
        while "searching common bobber":
            ...
            res = cv2.matchTemplate(
                processedImage, bobberCommon, cv2.TM_CCOEFF_NORMED)
            ...
            for point in zip(*loc[::-1]):
                ...
                startX = monitor['left'] + point[0] + \
                    random.randint(1, correctWidth)
                startY = monitor['top'] + point[1] + \
                    random.randint(1, correctHeight)
...
def detectFishUpPoint():
    with mss.mss() as screenshotManager:
        while "waiting fish up":
            ...
            res = cv2.matchTemplate(
                processedImage, bobberAngle, cv2.TM_CCOEFF_NORMED)
            loc = numpy.where(res >= 0.74)

            for point in zip(*loc[::-1]):
                correctWidth = bobberAngleWidth - 2 * errorFrameSize
                correctHeight = bobberAngleHeight - 2 * errorFrameSize
                fishUpX = monitor['left'] + point[0] + \
                    errorFrameSize + random.randint(1, correctWidth)
                fishUpY = monitor['top'] + point[1] + \
                    errorFrameSize + random.randint(1, correctHeight)
...
def handleLoop():
    ...
    while "Fishing":
        if coreStatus == Status.WORK:
            ...
            pyautogui.moveTo(
                x=startPoint[0], y=startPoint[1], duration=random.uniform(0.5, 1))
            ...
            if math.sqrt((startX - fishUpX) ** 2 + (startY - fishUpY) ** 2) < 40:
                fishUpX = startX
                fishUpY = startY
            pyautogui.click(x=fishUpX, y=fishUpY, button='right', duration=random.uniform(
                0.5, 1.5), tween=pyautogui.easeOutQuad)
